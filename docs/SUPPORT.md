...
- `<?php require 'sibling.php';` and `<?php include 'sibling.php';` as top-level
  literal-path statements in `phpc run`, resolved relative to the executing PHP
  file's directory.
...
- `phpc compile --emit-ir` rejects literal-path `require`/`include` with an
...
- `phpc compile --emit-exe` rejects literal-path `require`/`include` before
  ...
  This rejection does not execute the include or check whether the target file
  exists.
- Non-literal `require`/`include` path expressions, for example
...
- Minimal `phpc run` `.phpt` evaluator for parsed tests with `FILE` or `FILEEOF` plus literal `EXPECT` or `EXPECTF`; it normalizes line endings, trims expected and actual output before comparison to match php-src `run-tests.php`, resolves literal-path include/require statements in FILE bodies relative to the `.phpt` file's directory when run through the CLI, and classifies pass, fail, skip, xfail, unexpected-pass, unsupported matcher, and interpreter-error outcomes.
...
- CLI status probe: `phpc phpt-run <input.phpt>` runs a single `.phpt` through the existing `phpc run` evaluator and prints a machine-readable status report including the `.phpt` test name, source section kind, and expectation kind when present. It does not execute native code.
- Recorded php-src runnable subset: `tests/basic/001.phpt`, `tests/output/ob_001.phpt`, `tests/basic/gh15905.phpt`, `Zend/tests/bug47596.phpt`, and `Zend/tests/multibyte/bug68665.phpt` from the pinned PHP-8.3 tree execute through `phpc phpt-run` and currently report `pass` under `phpc_run`.
