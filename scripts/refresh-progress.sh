...
php_phpt_runner_summary = (
    "minimal FILE/FILEEOF exact-EXPECT/EXPECTF runner plus limited SKIPIF "
    "classification and base-path-aware literal include/require resolution "
    "integrated"
)
...
print(f"php_phpt_runner_summary={php_phpt_runner_summary!r}")
...
tmp="$(mktemp)"
{
  ...
  echo "| M5 PHP core .phpt harness | PHP-8.3 branch, ${php_phpt_total_display} \`.phpt\` files; ${php_phpt_runnable} ${php_phpt_recorded_label} | 6% | PHP-8.3 inventory pinned; ${php_phpt_subset_summary}; ${php_phpt_runner_summary} |"
  ...
  <main>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td>M5 PHP core harness</td><td><div class="bar"><span style="width:6%"></span></div>6%</td><td>PHP-8.3 branch, ${php_phpt_total_display} .phpt files; ${php_phpt_runnable} ${php_phpt_recorded_label}</td><td>Static inventory pinned; ${php_phpt_subset_summary}; ${php_phpt_runner_summary}</td></tr>
