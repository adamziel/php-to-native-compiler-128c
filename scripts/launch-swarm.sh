...
for lane in "${lanes[@]}"; do
  ...
  tmux send-keys -t "$session:$lane" "export CARGO_TARGET_DIR='${target_root}/${lane}' CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 RUST_TEST_THREADS=1; '$repo_root/scripts/worker-loop.sh' '$lane' '$worktree' '$prompt'" C-m
done
