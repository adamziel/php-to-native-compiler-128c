...
| 2026-05-26 | `INT-02` | `6d7075e` current-main port | Accept opt-in clean worktree worker preflight | Ported the focused `lane/INT-02-fresh-0436` worker hygiene slice: `scripts/verify-worker-env.sh` now supports `PHPC_REQUIRE_CLEAN_WORKTREE=1` to fail early when a lane worktree has uncommitted changes, and `scripts/test-worker-env.sh` covers clean and dirty fixture repositories. Verification: `scripts/test-worker-env.sh`, `scripts/local-gate.sh`, and `git diff --check` passed with `CARGO_TARGET_DIR=/home/ubuntu/phpc-targets/main-clean-worker-preflight`. | Keep this opt-in until generated progress/status jobs and active worker prompts are coordinated around clean-worktree expectations. |
| 2026-05-26 | `INT-03` | `ccf8134` current-main port | Accept worker lane-id basename preflight | Ported the focused `lane/INT-03-fresh-409` worker hygiene slice on top of the current clean-worktree preflight: `scripts/verify-worker-env.sh` now rejects mismatches between `PHPC_LANE_ID` and the worktree basename unless `PHPC_ALLOW_WORKTREE_LANE_MISMATCH=1` is set for synthetic fixtures, and worker/local-gate tests cover the new guard. Verification: `scripts/test-worker-env.sh`, `scripts/test-local-gate.sh`, `scripts/local-gate.sh`, and `git diff --check` passed with `CARGO_TARGET_DIR=/home/ubuntu/phpc-targets/main-lane-basename-preflight`. | Keep the mismatch escape hatch limited to tests and supervisor fixtures; do not make launcher preflight mandatory until lane branch/worktree setup is consistently aligned. |
